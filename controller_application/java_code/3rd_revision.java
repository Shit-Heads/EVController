...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int EXPECTED_MAX_RPM = 400;
    // Keeping the TOLERANCE_PERCENT but it will no longer be used for RPM consistency checks
    private static final double TOLERANCE_PERCENT = 0.5;
                    ...
                    if (line.startsWith("RPM")) {
                        ...
                        try {
                            ...
                            runOnUiThread(() -> {
                                ...
                                String status;
                                if (currentSpeed == 0) {
                                    // Motor speed set to 0
                                    status = (measuredRpm == 0) ? "Idle" : "Not Optimal";
                                } else {
                                    // Check if measuredRpm is within ±20 of expectedRpm
                                    int lowerBound = expectedRpm - 20;
                                    int upperBound = expectedRpm + 20;
                                    if (measuredRpm >= lowerBound && measuredRpm <= upperBound) {
                                        status = "Optimal";
                                    } else {
                                        status = "Not Optimal";
                                    }
